...
switch ($method) {
    ...
    case 'POST':
        try {
            ...
            if (($input['precio_a_convenir'] ?? 0) != 1 && !isset($input['precio'])) {
                jsonResponse(['error' => 'El precio es requerido (o marca "Precio a convenir")'], 400);
            }

            $slug = !empty($input['slug']) ? strtolower(trim($input['slug'])) : strtolower(str_replace(' ', '-', trim($input['nombre'])));
            $slug = preg_replace('/[^a-z0-9-]/', '', $slug);
            if (empty($slug)) $slug = 'producto-' . time();
            $baseSlug = $slug;
            $counter = 1;
            while (true) {
                $exists = $db->querySingle("SELECT id FROM productos WHERE slug = '" . addslashes($slug) . "'");
                if (!$exists) break;
                $slug = $baseSlug . '-' . $counter++;
            }

            $stmt = $db->prepare("INSERT INTO productos (nombre, slug, descripcion, precio, precio_oferta, imagen, categoria_id, destacado, activo, precio_a_convenir)
                VALUES (:nombre, :slug, :descripcion, :precio, :precio_oferta, :imagen, :categoria_id, :destacado, :activo, :precio_a_convenir)");
            $stmt->bindValue(':nombre', trim($input['nombre']), SQLITE3_TEXT);
            $stmt->bindValue(':slug', $slug, SQLITE3_TEXT);
            $stmt->bindValue(':descripcion', $input['descripcion'] ?? '', SQLITE3_TEXT);
            $stmt->bindValue(':precio', $input['precio'] ?? 0, SQLITE3_FLOAT);
